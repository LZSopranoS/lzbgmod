...
-- 一次遍历检测角色所有免疫状态：武器，法术等级，特殊，特定 Opcode 免疫
...
-- ========================================================	
	...
	if nameref and nameref ~= "" then
		table.insert(baseInfoList, "^0x00FFFF8BName^-: " .. nameref)
	end
...
if share > 0 then
    CurrentHP = EEex_ReadSignedWord(share + 0x57C, 0x0) -- 不要加 local
end
]]
	local share = EEex_GetActorShare(actorID)
	local CurrentHP = (share > 0) and EEex_ReadSignedWord(share + 0x57C, 0x0) or 0  --三元表达式写法

	--local CurrentHP = EEex_GetActorCurrentHP(actorID)
	--local CurrentHP = EEex_ReadWord(creatureData + 0x57C, 0x0)
	
	local MaxHP = EEex_Sprite_GetStat(object, 1)
	--local MaxHP = EEex_GetActorStat(actorID, 1)
	
	local hpStr
	-- 只有当两个值都正常读出时，才进行 %d 数字格式化
	if CurrentHP and MaxHP then
		hpStr = string.format("^0xFF8000FFHP^-: %d/%d", CurrentHP, MaxHP)
	else
		hpStr = "^0xFF8000FFHP^-: Unknown" -- 读失败时显示
	end
	table.insert(baseInfoList, hpStr)
...
-- ========================================================
	...
	table.insert(baseInfoList, acStr)
...
-- ========================================================
	...
	if classNum then
		-- 直接去外部字典中精准查表
		local foundName = classDict[classNum]
		if foundName then
			className = foundName
		end
	end
	table.insert(baseInfoList, "^0xFFDDA0DDClass^-: " .. className)
...
-- ========================================================
	...
	local levelStr
	if level1 then
		levelStr = "^0xFFDA70D6Level^-: " .. level1
		if level2 then levelStr = levelStr .. "/" .. level2 end
		if level3 then levelStr = levelStr .. "/" .. level3 end
	else
		levelStr = "^0xFFDA70D6Level^-: Unknown" -- 读不到主等级时，显示为文本
	end
	table.insert(baseInfoList, levelStr)
	
-- ========================================================
-- 最终拼接：在一行内输出所有基础信息
-- ========================================================
if #baseInfoList > 0 then
	-- 用 " | " 作为分隔符，输出效果如："Name: XX | HP: 99/100 | AC: -5 | Class: MAGE | Lv: 20"
	local finalInfoStr = table.concat(baseInfoList, " | ")
	Infinity_DisplayString(finalInfoStr)
end
...
if immunity.non_magicl == true then
	table.insert(weaponImmList, "Non-magical")  -- 非魔法武器
end

if immunity.magicl == true then
	table.insert(weaponImmList, "Magical")   -- 魔法武器
end

for enchant = 1, 6 do
	if immunity[enchant] == true then
		table.insert(weaponImmList, "+" .. enchant) -- 显示为 +1, +2等
	end
end

if #weaponImmList > 0 then
	local weaponStr = table.concat(weaponImmList, ", ")
	Infinity_DisplayString(string.format("^0xFF008BFFImmuneToWeapon^-: ^C%s^-", weaponStr))
end
...
for spelllv = 1, 9 do
	if immunity["sl" .. spelllv] == true then
		table.insert(immunesplList, "Lv" .. spelllv)
	end
end

if #immunesplList > 0 then
	local Immunespllvstr = table.concat(immunesplList, ", ")
	Infinity_DisplayString(string.format("^0xFFFF8C00ImmuneToSpellLevel^-: ^0xFFFFE6D0%s^-", Immunespllvstr))
end

-- ===========================================================
-- 3. 特定免疫显示 (Backstab, Turn Undead, Tracking, Time Stop)
-- ===========================================================
local specialImmList = {}
if immunity.backstab      then table.insert(specialImmList, "Backstab") end
if immunity.turnundead    then table.insert(specialImmList, "Turn Undead") end
if immunity.track         then table.insert(specialImmList, "Tracking") end
if immunity.timestop      then table.insert(specialImmList, "Time Stop") end

if #specialImmList > 0 then
	local specialStr = table.concat(specialImmList, ", ")
	Infinity_DisplayString(string.format("^0xFFDDA0DDImmuneToSpecial^-: ^4%s^-", specialStr))
end

-- ========================================================
-- 4. 特定 Opcode 免疫显示 (从字典匹配)
-- ========================================================
local immuneOpcodeList = {}
...
if #immuneOpcodeList > 0 then
	...
	Infinity_DisplayString(string.format("^0xFFDA70D6ImmuneToEffect^-: ^3%s^-", ImmuneOpcodestr))
end
...
--显示基本信息名字、血量……豁免、抗性需要function ME_UDToPtr(userdata)、function EEex_GetActorShare(actorID)支持
--显示免疫武器、法术、特殊、效果需要function EEex_IsSprite(actorID, allowDead)、function EEex_IterateActorEffects(actorID, func)支持